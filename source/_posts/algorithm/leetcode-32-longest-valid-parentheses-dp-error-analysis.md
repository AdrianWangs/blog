...
[LeetCode 32题 - 最长有效括号](https://leetcode.com/problems/longest-valid-parentheses/)要求我们找到给定字符串中由匹配括号组成的最长有效（格式正确且连续）括号子串的长度。
...
在第7步，也就是处理字符串最后一个字符 `)` 时，我们需要考虑它能否与某个左括号匹配。这时：
- `s[5] = ')'`
- `s[4] = ')'`（前一个字符也是右括号）
