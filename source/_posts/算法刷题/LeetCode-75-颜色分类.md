...
1.  `current = 0`, `nums[0] = 2` (蓝色)。
    ...
    -   数组变为 `[0, 0, 2, 1, 1, 2]`
    -   `right` 左移到索引 4。
    -   `current` (即 `i`) 保持在 0 (通过 `i--` 实现)，因为换过来的 `nums[0]` (现在是0) 需要重新检查。
    `nums = [0, 0, 2, 1, 1, 2]`
             ^           ^
            curr        right
            left
...
4.  `current = 2`, `nums[2] = 2` (蓝色)。
    ...
    -   数组变为 `[0, 0, 1, 1, 2, 2]`
    -   `right` 左移到索引 3。
    -   `current` (即 `i`) 保持在 2 (通过 `i--` 实现)，因为换过来的 `nums[2]` (现在是1) 需要重新检查。
    `nums = [0, 0, 1, 1, 2, 2]`
                   ^  ^
                  curr right
                  left
...
-   循环变量 `i` 就是我们的 `current` 标记，它从数组开头（索引0）一直扫描到 `right` 标记的位置。
